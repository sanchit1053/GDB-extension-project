    ...
    def to_string(self):
        ...
        if type == 1:
            ...
            for i in nodevector:
                ...
                info = ""
                for variable in S:
                    info += '"' + str(variable) + '":"' + str(
                        i[variable]) + '", '
                info = info[0:-2]
                info = "{" + info + "}"
                data.append({
                    "data": {
                        "id": int(i[config[type]["keys"]]),
                        "color": normalcolor,
                        "info": info
                    }
                })
            ...
            for i in edgevector:
                ...
                #(key1, key2) ,
                edges += "(" + str(
                    i["first"][config[type]["keys"]]) + "," + str(
                        i["second"][config[type]["keys"]]) + '), '
                ...
                data.append({
                    "data": {
                        "id":
                        str(i["first"][config[type]["keys"]]) +
                        str(i["second"][config[type]["keys"]]),
                        "source":
                        int(i["first"][config[type]["keys"]]),
                        "target":
                        int(i["second"][config[type]["keys"]])
                    }
                })
            ...
            for node in nodevector:
                ...
                for neighbour in adjacent:
                    edges += "(" + str(node[config[type]["keys"]]) + "," + str(
                        neighbour[config[type]["keys"]]) + '), '
                    data.append({
                        "data": {
                            "id":
                            str(node[config[type]["keys"]]) +
                            str(neighbour[config[type]["keys"]]),
                            "source":
                            int(node[config[type]["keys"]]),
                            "target":
                            int(neighbour[config[type]["keys"]])
                        }
                    })
